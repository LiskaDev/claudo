...
- **Ring arc** — tracks the *total conversation context* (all messages combined). Green = plenty of room, amber = over 40k tokens, red = over 120k tokens (time to start a new chat). Think of it as a fuel gauge for the conversation.
...
- **圆环弧线** — 追踪整个对话的上下文用量（所有消息加在一起）。绿色 = 空间充裕，黄色 = 超过 4 万 token，红色 = 超过 12 万 token（该开新对话了）。像汽车的油量表。
